...
class NavigationDivider extends StatelessWidget implements NavigationBarItem {
  ...
  Widget build(BuildContext context) {
    ...
    if (direction == Axis.vertical) {
      child = Divider(
        indent: math.max(0.0, -parentPadding.left),
        endIndent: math.max(0.0, -parentPadding.right),
        thickness: thickness ?? (1 * scaling),
        color: color ?? theme.colorScheme.muted,
      );
    } else {
      child = VerticalDivider(
        indent: math.max(0.0, -parentPadding.top),
        endIndent: math.max(0.0, -parentPadding.bottom),
        thickness: thickness ?? (1 * scaling),
        color: color ?? theme.colorScheme.muted,
      );
    }
