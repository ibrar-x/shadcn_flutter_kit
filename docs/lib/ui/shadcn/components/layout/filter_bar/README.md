...
### Logic-only import (no FilterBar UI)

```dart
import 'package:<your_app>/ui/shadcn/layout/filter_bar/filter_bar_logic.dart';
```
...
### Logic-only minimal example

```dart
final state = const FilterState()
    .setCustomValue('status', 'paid');
```
...
### Pattern: Sheet presentation with groups
...
### Pattern: Explicit model bindings
